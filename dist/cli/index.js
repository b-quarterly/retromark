    ...
    try {
        ...
        if (options.chapter) {
            ...
            const files = fs.existsSync(contentDir)
                ...
                : [];
            const chapterCount = files.filter(f => f.match(/^\d{2}-/) && fs.statSync(path.join(contentDir, f)).isFile()).length;
...
function setupCLI() {
    ...
    program.command('build')
        ...
        .action((options) => {
        buildCommand(options.input, options.output, options.config, {
            theme: options.theme,
            toc: options.toc,
            math: options.math
        });
    ...
    program.command('serve')
        ...
        .action((options) => {
        serveCommand(options.input, options.output, options.config, parseInt(options.port, 10));
    });
    // Init Command
